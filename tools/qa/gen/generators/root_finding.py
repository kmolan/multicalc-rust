...
def _write_scalar(out, meta, case, problem, solver, params, root_val, probe, tol,
                  operation, equation, bracket=None, start=None):
    ...
    schema.write_fixture(out, "root_finding", case, meta, {"f64": schema.tol(*tol)}, inputs, expected,
                         equation=equation, operations=[operation])


def _write_system(out, meta, case, problem, params, start, root_val, tol, operation, equation):
...
def run(out, seed):
    ...
    _write_scalar(out, meta, "sigmoid_damped_newton", "root_sigmoid", "damped_newton", {},
                  ...
                  operation="Damped Newton", equation="x/√(1+x²) = 0")
    ...
    _write_system(out, meta, "two_link_ik", "sys_two_link", tl_params, [0.4, 0.9], tl_root, tol=(1e-9, 1e-9),
                  operation="Newton system, 2×2", equation="two-link IK: tip at (pₓ, pᵧ)")
    ...
    _write_system(out, meta, "circle_hyperbola", "sys_circle_hyperbola", {}, [1.5, 0.8], ch_root, tol=(1e-9, 1e-9),
                  operation="Newton system, 2×2", equation="[x²+y²−4, x·y−1] = 0")
    ...
    _write_system(out, meta, "equilibrium_3x3", "sys_equilibrium", {}, [0.5, 0.25, 0.25], eq_root, tol=(1e-8, 1e-8),
                  operation="Newton system, 3×3", equation="[x+y+z−1, y−1.25x², z−5x·y] = 0")
