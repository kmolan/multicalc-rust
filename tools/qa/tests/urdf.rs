...
fn ingested_urdf_models_match_pinocchio() {
    for fx in load_dir("urdf") {
        match fx.case.as_str() {
            "panda_urdf_tree" => check_panda_tree(&fx),
            "panda_urdf_forward_kinematics" => check_panda_forward_kinematics(&fx),
...
fn check_panda_tree(fx: &Fixture) {
    let case = fx.case.as_str();
    let path = third_party().join(fx.inputs["model_file"].as_str());
    ...
    assert_eq!(
        names.join(" "),
        fx.expected["joint_names"].as_str(),
        "{case} joint names"
    );
    ...
    assert_eq!(
        kinds,
        fx.expected["joint_kinds"].as_str(),
        "{case} joint kinds"
    );

    let lower = fx.expected["lower_limits"].as_vector();
    let upper = fx.expected["upper_limits"].as_vector();
    ...
    for (index, joint) in joints.iter().enumerate() {
        ...
        assert!(close(got_lower, lower[index], t), "{ctx} lower");
        assert!(close(got_upper, upper[index], t), "{ctx} upper");
    }
    ...
    assert_eq!(
        with_mass as i64,
        fx.expected["links_with_mass"].as_int(),
    ...
    assert_eq!(
        follower.name(),
        fx.expected["mimicking_joint"].as_str(),
    ...
    assert_eq!(
        mimic.joint(),
        fx.expected["mimic_joint"].as_str(),
        "{case} mimic joint"
    );
    assert_scalar(
        mimic.multiplier(),
        &fx.expected["mimic_multiplier"],
        t,
        &format!("{case} mimic multiplier"),
    );
    assert_scalar(
        mimic.offset(),
        &fx.expected["mimic_offset"],
        t,
...
fn check_panda_forward_kinematics(fx: &Fixture) {
    let case = fx.case.as_str();
    let path = third_party().join(fx.inputs["model_file"].as_str());
    let model = multicalc_robot_model::urdf::load_path(&path)
        .unwrap_or_else(|e| unreachable!("{case}: loading {path:?}: {e}"));
    ...
    let frame_count = fx.expected["frame_count"].as_int() as usize;
    let tree = model
        .kinematic_tree_to::<10, 10>("panda_hand")
        .unwrap_or_else(|e| unreachable!("{case}: building the chain to panda_hand: {e}"));
    ...
    let (_, arm_joint_count, configurations) = fx.inputs["configurations"].as_matrix();
    let (_, _, translations) = fx.expected["translations"].as_matrix();
    let (_, _, quaternions) = fx.expected["quaternions"].as_matrix();
    let configuration_count = fx.expected["configuration_count"].as_int() as usize;
    ...
    for run in 0..configuration_count {
        ...
        let state = tree
            ...
            .unwrap_or_else(|e| unreachable!("{case}: forward kinematics for run {run}: {e}"));
        ...
        for (slot, link) in links.iter().enumerate() {
            ...
            for axis in 0..3 {
                assert!(
                    close(pose.translation()[axis], translations[base + axis], t),
            ...
            for component in 0..4 {
                assert!(
                    close(flip * got[component], want_flip * want[component], t),
