...
#[must_use]
fn wall() -> DynamicOccupancyGrid {
    let mut grid = DynamicOccupancyGrid::try_new(15, 12, 1.0, [-5.0, -5.25]).unwrap();
    for row in 0..grid.rows() {
        grid.set_cell(row, 7, true);
    }
    grid
}
...
fn noiseless_scan_matches_closed_form() {
    ...
    let lidar = Lidar2d::<3>::new(PI / 2.0, 10.0, 0.0, 0.0).unwrap();
...
fn empty_map_reads_as_no_return() {
    let lidar = Lidar2d::<5>::new(PI / 2.0, 10.0, 0.0, 0.0).unwrap();
    let mut rng = Pcg32::seed_from_u64(1);
    let empty = DynamicOccupancyGrid::try_new(15, 12, 1.0, [-5.0, -5.25]).unwrap();
...
fn certain_dropout_drops_every_beam() {
    let lidar = Lidar2d::<5>::new(PI / 2.0, 10.0, 0.0, 1.0).unwrap();
    let mut rng = Pcg32::seed_from_u64(1);
    let scan = lidar.simulate(&wall(), [0.0, 0.0, 0.0], &mut rng);
    assert!(scan.iter().all(|range| range.is_infinite()));
}
...
fn a_fixed_seed_reproduces_the_scan() {
    let lidar = Lidar2d::<9>::new(PI / 2.0, 10.0, 0.03, 0.01).unwrap();
    let map = wall();
...
fn range_noise_has_about_the_right_spread() {
    // The middle beam of three points straight at the wall. 10 000 scans from a single seeded
    // generator, so this is a regression check rather than a flaky statistical test.
    let lidar = Lidar2d::<3>::new(PI / 2.0, 10.0, 0.03, 0.0).unwrap();
    let map = wall();
