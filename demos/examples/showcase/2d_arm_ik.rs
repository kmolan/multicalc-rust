//! 1 kHz 2D arm IK (optimization showcase).
//!
...
//! Run with: cargo run --release -p multicalc-demos --example 2d_arm_ik
...
fn main() -> Result<(), VizError> {
    if cfg!(debug_assertions) {
        eprintln!(
            "WARNING: debug build — timing numbers are meaningless. \
             Re-run with: cargo run --release -p multicalc-demos --example 2d_arm_ik"
        );
    }

    let mut rr = RerunSink::live("multicalc-demos/2d-arm-ik")?;
    ...
    loop {
            ...
            && let Some(s) = solve_ring.summary()
        {
            ...
            let md = format!(
                "## 2d_arm_ik — multicalc live demo\n\
                 ### full IK solve (Levenberg–Marquardt, exact autodiff Jacobian): median {:.0} µs · p99 {:.0} µs ({:.1} % of the 1 ms tick)\n\
                 ### accuracy: position error ‖ee−target‖ = {:.3} µm\n\
                 ### LM residual evaluations: {} this tick, {:.1} average",
                s.median,
                s.p99,
                s.p99 / 10.0,
                last_pos_residual * 1e6,
                evals_live,
                evals_avg,
            );
