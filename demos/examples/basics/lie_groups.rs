...
fn main() {
    ...
    let p = *rz.act(Vector::new([1.0, 0.0, 0.0])).as_array();
    ...
    for i in 0..3 {
        report(&format!("phi[{i}]"), back.as_array()[i], phi.as_array()[i]);
    }
    ...
    report("x", q.as_array()[0], 1.0);
    report("y", q.as_array()[1], 3.0);
    report("z", q.as_array()[2], 3.0);
    ...
    report("angle (rad)", mid.log().as_array()[2], FRAC_PI_2 / 2.0);
    ...
    report("d x / d theta", out.as_array()[0].deriv, 0.0);
    report("d y / d theta", out.as_array()[1].deriv, 1.0);
    report("d z / d theta", out.as_array()[2].deriv, 0.0);
