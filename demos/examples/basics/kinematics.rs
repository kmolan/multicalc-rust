//! Differential-drive kinematics: wheel/chassis maps and their round trip, exact SE(2) odometry
...
fn main() {
    ...
    // (1) Wheel rates to chassis motion. Equal rates drive straight; opposite rates spin in place.
    ...
    let spin = dd.forward(WheelRates::new(-10.0, 10.0));
    ...
    let wheels = WheelRates::new(7.5, -2.25);
    let back = dd.inverse(dd.forward(wheels));
    println!("\nRound trip: wheels -> chassis -> wheels");
    ...
    let pose = integrate(SE2::identity(), ChassisRate::new(v, w).integrate_over(t));
    ...
    for sign in [1.0, -1.0] {
        let rates = dd.inverse(ChassisRate::new(0.36, w * sign));
        let (left_m, right_m) = (
            rates.left() * dd.wheel_radius() * dt,
            rates.right() * dd.wheel_radius() * dt,
        );
        for _ in 0..n {
            let deltas = dd.wheel_deltas_from_travel(left_m, right_m);
            figure_eight = dd.odometry_step(figure_eight, deltas);
        }
    }
    ...
    let step = integrate(
        ...
        ChassisRate::new(Dual::variable(v), Dual::constant(w)).integrate_over(Dual::constant(t)),
    );
