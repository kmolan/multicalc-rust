...
use crate::defaults::{bad_attribute, parse_vector3, parse_vector4, parse_vector6};
...
impl Orientation {
    ...
    pub(crate) fn read(node: Node) -> Result<Self, MjcfError> {
        ...
        if stated > 1 {
            return Err(MjcfError::MultipleOrientations {
...
fn direction(node: Node, attribute: &'static str, stated: [f64; 3]) -> Result<Vector3D, MjcfError> {
    Vector::new(stated)
        .try_normalized()
...
#[must_use]
fn unreadable(node: Node, attribute: &'static str) -> MjcfError {
    bad_attribute(
        node,
        attribute,
