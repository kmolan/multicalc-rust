...
- [Rigid-body dynamics and rotor mixing](#rigid-body-dynamics-and-rotor-mixing)
...
Errors: every fallible call returns [`PolynomialError`](#error-handling). Demo:
...
The trajectory comes back as a [`PiecewisePolynomial`](#polynomials) from the polynomial module, so
its calls report a [`PolynomialError`](#error-handling) rather than a `MotionError`, and it calls a
