...
handed, so a NaN or an infinity goes straight into the filter and whatever falls out is the answer.
What that costs runs from a single spoiled sample, through one window of them, to a filter that is
ruined until it is reset — and for two of them the failure is silent rather than loud. Every filter
also has a checked entry point that refuses a non-finite sample before it can do any of that. The
module's own documentation carries a table of the exact cost and the checked call for each filter;
[Non-finite samples](#non-finite-samples) below shows how to use them.
...
question of when, not whether. The unchecked call is the one to use when the samples have already
been validated — it costs no branch, which is what a 1 kHz loop wants. The checked call is for
everything else.
...
let timestep = 0.001_f64;
let shape = BiquadCoefficients::low_pass(50.0, 0.70710678, timestep).unwrap();

// Unchecked, one bad sample ruins the filter for good: the value enters the feedback path, and
// every later output is a NaN no matter how clean the input becomes.
let mut spoiled = Biquad::new(shape);
let _ = spoiled.filter(1.0);
let _ = spoiled.filter(f64::NAN);
assert!(spoiled.filter(1.0).is_nan());

// Checked, the sample is refused and the filter is left exactly where it was, so the loop can
// drop the reading and carry on with the next one.
...
assert!(guarded.filter_checked(1.0).unwrap().is_finite());

// `RunningMedian` is the exception. An infinity is an ordinary wild reading to a median, so it is
// let through and dropped by the sort; only a NaN is refused, because that one would silently
// move the answer to a wrong finite number rather than an obvious one.
let mut median = RunningMedian::<5, f64>::new().unwrap();
...
Two habits make this work. Pick one entry point per filter and stay with it: a checked call promises
that *this* sample will not spoil the filter, not that the output is finite, so a filter already
ruined through the unchecked call keeps returning NaN from checked calls too. And when a filter has
been spoiled, `reset` is what brings it back — nothing else does.

The two silent failures are worth singling out, because no check downstream will catch them. A NaN
reaching `RunningMedian` shifts its answer to a plausible finite number that is simply wrong, and a
