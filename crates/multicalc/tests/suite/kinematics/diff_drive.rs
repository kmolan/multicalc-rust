...
use multicalc::kinematics::{ChassisDelta, ChassisRate, DiffDrive, WheelDeltas, WheelRates};
...
/// the wheel/chassis magnitudes, and the reconstruction cancels two terms of that size to recover a
...
fn rand_geometry(rng: &mut StdRng) -> DiffDrive<f64> {
    DiffDrive::new(rng.gen_range(0.01..0.5), rng.gen_range(0.05..1.0)).unwrap()
}

fn rand_wheels(rng: &mut StdRng) -> WheelRates<f64> {
    WheelRates::new(rng.gen_range(-20.0..20.0), rng.gen_range(-20.0..20.0))
}

fn rand_chassis(rng: &mut StdRng) -> ChassisRate<f64> {
    ChassisRate::new(rng.gen_range(-5.0..5.0), rng.gen_range(-5.0..5.0))
}

/// Error scales for reconstructing a chassis pair from the wheel pair it maps to. Recovering the
...
fn chassis_scales(dd: DiffDrive<f64>, w: WheelRates<f64>) -> (f64, f64) {
...
#[test]
fn wheels_to_chassis_to_wheels_round_trip() {
...
#[test]
fn chassis_to_wheels_to_chassis_round_trip() {
    let mut rng = StdRng::seed_from_u64(0x9966_5678);
    for _ in 0..1000 {
        let dd = rand_geometry(&mut rng);
        let c = rand_chassis(&mut rng);
        let w = dd.inverse(c);
        let back = dd.forward(w);
        let (linear_scale, angular_scale) = chassis_scales(dd, w);
...
#[test]
fn wheel_deltas_round_trip() {
    let mut rng = StdRng::seed_from_u64(0x9966_abcd);
    for _ in 0..1000 {
        let dd = rand_geometry(&mut rng);
        let w = rand_wheels(&mut rng);
        let d = WheelDeltas::new(w.left(), w.right());
        let back = dd.inverse_delta(dd.forward_delta(d));
...
#[test]
fn chassis_delta_round_trip() {
    let mut rng = StdRng::seed_from_u64(0x9966_ef01);
    for _ in 0..1000 {
        let dd = rand_geometry(&mut rng);
        let c = rand_chassis(&mut rng);
        let d = ChassisDelta::new(c.linear(), c.angular());
        let w = dd.inverse_delta(d);
        let back = dd.forward_delta(w);
        let (linear_scale, angular_scale) =
            chassis_scales(dd, WheelRates::new(w.left(), w.right()));
...
fn straight_line_has_zero_angular() {
    ...
    for x in [0.0, 1.0, -3.5, 20.0] {
        assert_eq!(dd.forward(WheelRates::new(x, x)).angular(), 0.0);
    }
}
...
fn spin_in_place_has_zero_linear() {
    let dd = DiffDrive::new(0.036_f64, 0.235).unwrap();
    for x in [0.0, 1.0, -3.5, 20.0] {
        assert_eq!(dd.forward(WheelRates::new(-x, x)).linear(), 0.0);
    }
}
...
fn known_values() {
    ...
    let straight = dd.forward(WheelRates::new(10.0, 10.0));
    assert!((straight.linear() - 0.36).abs() < TOL);
    assert!((straight.angular() - 0.0).abs() < TOL);

    let turning = dd.forward(WheelRates::new(0.0, 10.0));
...
fn new_rejects_non_positive_radius() {
    assert_eq!(
        DiffDrive::new(0.0_f64, 0.235),
        Err(KinematicsError::NonPositiveParameter)
    );
    assert_eq!(
        DiffDrive::new(-1.0_f64, 0.235),
        Err(KinematicsError::NonPositiveParameter)
    );
}
...
fn new_rejects_non_positive_wheelbase() {
    assert_eq!(
        DiffDrive::new(0.036_f64, 0.0),
        Err(KinematicsError::NonPositiveParameter)
    );
    assert_eq!(
        DiffDrive::new(0.036_f64, -1.0),
        Err(KinematicsError::NonPositiveParameter)
    );
}
...
fn new_rejects_nan() {
    ...
    assert_eq!(
        DiffDrive::new(f64::NAN, 0.235),
        Err(KinematicsError::NonFinite)
    );
    assert_eq!(
        DiffDrive::new(0.036, f64::NAN),
        Err(KinematicsError::NonFinite)
    );
}
...
fn new_rejects_infinite() {
    assert_eq!(
        DiffDrive::new(f64::INFINITY, 0.235),
        Err(KinematicsError::NonFinite)
    );
    assert_eq!(
        DiffDrive::new(0.036, f64::NEG_INFINITY),
        Err(KinematicsError::NonFinite)
    );
}
...
fn to_tangent_has_zero_lateral() {
    let xi = ChassisRate::new(1.0, 2.0).to_tangent();
    assert_eq!(xi, Vector::new([1.0, 0.0, 2.0]));
}

#[test]
fn project_tangent_discards_lateral() {
    let c = ChassisRate::project_tangent(Vector::new([1.0, 9.9, 2.0]));
...
fn integrate_over_scales_both() {
    let d = ChassisRate::new(2.0, 4.0).integrate_over(0.5);
...
fn travel_converters_round_trip() {
    ...
    for _ in 0..1000 {
        ...
        let (left, right) = dd.wheel_travel(dd.wheel_deltas_from_travel(a, b));
...
fn round_trips_hold_at_f32() {
    let dd = DiffDrive::new(0.036_f32, 0.235).unwrap();
    let bound = 8.0 * f32::EPSILON;

    let w = WheelRates::new(1.5_f32, -2.5);
    let back = dd.inverse(dd.forward(w));
    assert!((back.left() - w.left()).abs() <= bound * 2.5);
    assert!((back.right() - w.right()).abs() <= bound * 2.5);

    let c = ChassisRate::new(0.3_f32, 0.9);
