...
fn view_reads_the_same_entries_as_the_matrix() {
    ...
    for row in 0..3 {
        for column in 0..4 {
            assert_eq!(view.get(row, column).ok(), matrix.get(row, column));
        }
    }
    assert_eq!(view.get(3, 0), Err(LinalgError::OutOfBounds));
    assert_eq!(view.get(0, 4), Err(LinalgError::OutOfBounds));
}
...
fn from_row_major_slice_checks_the_length() {
    ...
    assert!(MatrixView::<1, 5>::from_row_major_slice(&buffer).is_ok());
    assert_eq!(
        MatrixView::<3, 2>::from_row_major_slice(&buffer).unwrap_err(),
        LinalgError::OutOfBounds
    );
    assert!(VectorView::<6>::from_slice(&buffer).is_err());
}
...
fn submatrix_rejects_a_block_that_runs_off_an_edge() {
    ...
    assert_eq!(
        matrix.view().submatrix::<2, 2>(2, 0).unwrap_err(),
        LinalgError::OutOfBounds
    );
    assert!(matrix.view().submatrix::<2, 2>(0, 2).is_err());
    assert!(matrix.view().submatrix::<4, 1>(0, 0).is_err());
    assert!(matrix.view().submatrix::<1, 1>(usize::MAX, 0).is_err());
}
...
fn submatrix_composes_with_transpose() {
    ...
    let transposed_first = matrix.view().transposed().submatrix::<2, 2>(1, 1).unwrap();
    let blocked_first = matrix.view().submatrix::<2, 2>(1, 1).unwrap().transposed();
...
fn owning_submatrix_matches_the_view() {
    ...
    assert_eq!(
        matrix.submatrix::<2, 2>(2, 2).unwrap_err(),
        LinalgError::OutOfBounds
    );
}
...
fn set_submatrix_writes_a_block_and_reports_a_bad_corner() {
    ...
    assert_eq!(matrix.set_submatrix(1, 1, block.view()), Ok(()));
    assert_eq!(
        matrix.into_array(),
    ...
    let before = matrix;
    assert_eq!(
        matrix.set_submatrix(2, 2, block.view()),
...
fn row_and_column_views_match_the_copying_accessors() {
    ...
    assert!(matrix.view().row(3).is_err());
    assert!(matrix.view().column(4).is_err());
}
...
fn diagonal_view_walks_the_shorter_side() {
    ...
    assert_eq!(
        wide.view().diagonal::<4>().unwrap_err(),
        LinalgError::OutOfBounds
    );
}
...
fn a_writable_diagonal_writes_the_same_entries_the_read_only_one_reads() {
    ...
    assert_eq!(
        matrix.view().diagonal::<3>().unwrap().to_vector(),
        Vector::new([1.0, 1.0, 1.0])
    );

    let mut wide = counted::<2, 4>();
    assert!(wide.view_mut().diagonal::<4>().is_err());
    wide.view_mut().diagonal::<2>().unwrap().fill(9.0);
    assert_eq!(
        wide.into_array(),
...
fn segment_narrows_a_vector_view() {
    ...
    assert_eq!(
        vector.view().segment::<3>(2).unwrap_err(),
        LinalgError::OutOfBounds
    );
    assert!(vector.view().segment::<1>(usize::MAX).is_err());
}
...
fn a_writable_segment_narrows_the_same_way() {
    ...
    assert!(vector.view_mut().segment::<3>(2).is_err());
    assert!(vector.view_mut().segment::<1>(usize::MAX).is_err());
}
...
fn writes_through_a_view_land_in_the_matrix() {
    ...
    *matrix.view_mut().get_mut(0, 1).unwrap() = 5.0;
    ...
    *matrix.view_mut().get_mut(1, 0).unwrap() = 6.0;
    ...
    assert_eq!(
        matrix.view_mut().get_mut(2, 0).unwrap_err(),
        LinalgError::OutOfBounds
    );
}
...
fn split_rows_at_yields_two_simultaneously_writable_halves() {
    ...
    *bottom.get_mut(0, 1).unwrap() = 2.0;
    *bottom.get_mut(1, 0).unwrap() = 3.0;
...
fn split_rows_at_rejects_a_partition_that_does_not_add_up() {
    ...
    assert!(matrix.view_mut().split_rows_at::<1, 1>().is_err());
    assert!(matrix.view_mut().split_rows_at::<2, 2>().is_err());
    assert!(matrix.view_mut().split_rows_at::<0, 0>().is_err());
}
...
fn split_cols_at_rejects_a_row_major_view_and_a_bad_partition() {
    ...
    assert_eq!(
        matrix.view_mut().split_cols_at::<1, 2>().unwrap_err(),
        LinalgError::OutOfBounds
    );
    assert!(
        matrix
...
fn read_only_splits_need_no_layout_and_cover_every_entry() {
    ...
    let (top, bottom) = matrix.view().split_rows_at::<1, 1>().unwrap();
    ...
    assert!(matrix.view().split_rows_at::<1, 2>().is_err());
    assert!(matrix.view().split_cols_at::<1, 1>().is_err());
}
...
fn vector_split_at_rejects_a_bad_partition_or_a_strided_view() {
    let mut vector = Vector::<4>::zeros();
    assert_eq!(
        vector.view_mut().split_at::<1, 1>().unwrap_err(),
        LinalgError::OutOfBounds
    );
    ...
    assert!(column.split_at::<1, 2>().is_err());
}
...
fn empty_shapes_are_viewable_and_never_dereference() {
    ...
    assert_eq!(view.get(0, 0), Err(LinalgError::OutOfBounds));
    assert_eq!(view.to_matrix(), empty);
    assert_eq!(view.column(0).unwrap().len(), 0);
    assert!(view.column(3).is_err());
...
fn every_out_of_range_subscript_is_reported_rather_than_panicking() {
    ...
    assert_eq!(
        matrix.view().transposed().get(0, 2),
        Err(LinalgError::OutOfBounds),
    ...
    assert_eq!(
        writable.view_mut().get_mut(0, 2),
        Err(LinalgError::OutOfBounds)
    );

    let vector = Vector::new([1.0, 2.0]);
    assert_eq!(vector.view().get(2), Err(LinalgError::OutOfBounds));

    let mut writable_vector = Vector::new([1.0, 2.0]);
    assert_eq!(
        writable_vector.view_mut().get_mut(2),
        Err(LinalgError::OutOfBounds)
    );
}
...
fn views_work_over_a_non_numeric_element_type() {
    ...
    assert_eq!(matrix.view().transposed().get(0, 1), Ok(&'c'));
    assert_eq!(
...
fn views_pass_non_finite_entries_through_untouched() {
    ...
    assert!(transposed.get(0, 0).unwrap().is_nan());
    assert_eq!(transposed.get(0, 1), Ok(&f64::INFINITY));
    assert_eq!(transposed.get(1, 0), Ok(&1.0));
    assert_eq!(transposed.get(1, 1), Ok(&f64::NEG_INFINITY));
...
fn check_transposed_view_matches_owned<const ROWS: usize, const COLS: usize>(
    ...
    for row in 0..COLS {
        for column in 0..ROWS {
            prop_assert_eq!(view.get(row, column).ok(), matrix.get(column, row));
        }
    }
    Ok(())
}
...
fn check_submatrix_matches_manual_indexing<const ROWS: usize, const COLS: usize>(
    ...
    let Ok(block) = matrix.view().submatrix::<2, 2>(top, left) else {
    ...
    Matrix<ROWS, COLS>: Copy,
{
    ...
    let Ok((top, bottom)) = scratch.view_mut().split_rows_at::<TOP, 1>() else {
        prop_assert_ne!(TOP + 1, ROWS);
        return Ok(());
    };
    for row in 0..TOP {
        for column in 0..COLS {
            prop_assert_eq!(top.get(row, column).ok(), matrix.get(row, column));
        }
    }
    for column in 0..COLS {
        prop_assert_eq!(bottom.get(0, column).ok(), matrix.get(TOP, column));
    }
    Ok(())
}
...
fn check_split_cols_covers_every_entry<const ROWS: usize, const COLS: usize, const LEFT: usize>(
    ...
    let Ok((left, right)) = matrix.view().split_cols_at::<LEFT, 1>() else {
        prop_assert_ne!(LEFT + 1, COLS);
        return Ok(());
    };
    for row in 0..ROWS {
        for column in 0..LEFT {
            prop_assert_eq!(left.get(row, column).ok(), matrix.get(row, column));
        }
        prop_assert_eq!(right.get(row, 0).ok(), matrix.get(row, LEFT));
    }
    Ok(())
}
