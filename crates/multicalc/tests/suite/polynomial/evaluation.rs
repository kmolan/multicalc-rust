...
fn evaluate_with_derivatives_matches_repeated_derivative() {
    let p = Polynomial::new(MIXED);
    let x = 1.7_f64;
    let [value, first, second, third] = p.evaluate_with_derivatives(x);

    let first_derivative = p.derivative();
    let second_derivative = first_derivative.derivative();
    let third_derivative = second_derivative.derivative();

    assert!((value - p.evaluate(x)).abs() < 1e-12);
...
fn evaluate_is_accurate_for_f32() {
    ...
    let p: Polynomial<3, f32> = Polynomial::new([1.0, -2.0, 3.0]);
    assert!((p.evaluate(2.0) - 9.0).abs() < 1e-5);
}
...
fn divide_then_multiply_returns_the_original() {
    // x⁴ - 2x³ + 3x - 5, over x² + 1.
    let p: Polynomial<5> = Polynomial::new([-5.0, 3.0, 0.0, -2.0, 1.0]);
    let divisor: Polynomial<3> = Polynomial::new([1.0, 0.0, 1.0]);

    let (quotient, remainder) = p.divide::<3, 5, 3>(&divisor).unwrap();
    let rebuilt =
        quotient.multiply_into::<3, 5>(&divisor).unwrap() + remainder.try_resize::<5>().unwrap();

    for (returned, original) in rebuilt.coefficients().iter().zip(p.coefficients().iter()) {
...
fn shift_argument_matches_evaluating_shifted() {
    let p: Polynomial<4> = Polynomial::new([2.0, -3.5, 0.75, 4.0]);
    let offset = 1.3;

    let shifted = p.shift_argument(offset);
    for x in [-2.0, 0.0, 0.4, 3.1] {
        assert!((shifted.evaluate(x) - p.evaluate(x + offset)).abs() < 1e-12);
    }
}
...
fn scale_argument_matches_evaluating_scaled() {
    ...
    let scaled = p.scale_argument(factor);
    for x in [-2.0, 0.0, 0.4, 3.1] {
        assert!((scaled.evaluate(x) - p.evaluate(factor * x)).abs() < 1e-12);
    }
}
...
fn derivative_matches_finite_difference() {
    ...
    let approximate = (p.evaluate(x + step) - p.evaluate(x - step)) / (2.0 * step);
    assert!((p.derivative().evaluate(x) - approximate).abs() < 1e-6);
}

#[test]
fn derivative_leaves_top_coefficient_zero() {
    let p: Polynomial<4> = Polynomial::new([1.0, 2.0, 3.0, 4.0]);
    assert_eq!(p.derivative().coefficient(3), Some(0.0));
}
...
fn nth_derivative_past_the_degree_is_zero() {
    ...
    assert_eq!(p.nth_derivative(3).coefficients(), &[24.0, 0.0, 0.0, 0.0]);
    assert!(p.nth_derivative(4).is_zero());
    assert!(p.nth_derivative(100).is_zero());
}

#[test]
fn definite_integral_matches_known_areas() {
    // 2 - 3.5x + 0.75x² + 4x³, whose area from 0 to 1 is 2 - 1.75 + 0.25 + 1.
    let p: Polynomial<5> = Polynomial::new([2.0, -3.5, 0.75, 4.0, 0.0]);
    assert!((p.definite_integral(0.0, 1.0) - 1.5).abs() < 1e-12);

    // Covering the range in two pieces gives the same area as covering it in one.
    let (lower, split, upper) = (-0.6, 1.0, 2.3);
    let whole = p.definite_integral(lower, upper);
    let pieces = p.definite_integral(lower, split) + p.definite_integral(split, upper);
    assert!((whole - pieces).abs() < 1e-12);

    // Swapping the ends flips the sign.
    assert!((p.definite_integral(upper, lower) + whole).abs() < 1e-12);
