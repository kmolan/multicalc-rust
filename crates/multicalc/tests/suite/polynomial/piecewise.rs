//! Piecewise polynomial tests: locating a parameter, clamping at the ends, slopes and areas
//! against the shared parameter, and what the accessors report.
...
fn clamps_before_the_start_and_after_the_end() {
    ...
    let at_start = curve.evaluate(0.0).unwrap();
    let at_end = curve.evaluate(total).unwrap();
    for parameter in [-1.0, -1000.0] {
        let before = curve.evaluate(parameter).unwrap();
        assert!((before[0] - at_start[0]).abs() < 1e-12 && (before[1] - at_start[1]).abs() < 1e-12);
    }
    for parameter in [total + 1e-9, total + 1000.0] {
        let after = curve.evaluate(parameter).unwrap();
        assert!((after[0] - at_end[0]).abs() < 1e-12 && (after[1] - at_end[1]).abs() < 1e-12);
    }
}
...
fn derivatives_match_finite_differences() {
    ...
    for parameter in [0.9, 2.3] {
        ...
        let ahead = curve
            .evaluate_with_derivatives::<2>(parameter + step)
            .unwrap();
        let behind = curve
            .evaluate_with_derivatives::<2>(parameter - step)
            .unwrap();
        ...
        for axis in 0..2 {
            let value_ahead = ahead[0][axis];
            let value_behind = behind[0][axis];
            let approximate_slope = (value_ahead - value_behind) / (2.0 * step);
            assert!((slope[axis] - approximate_slope).abs() < 1e-5);

            let approximate_bend = (ahead[1][axis] - behind[1][axis]) / (2.0 * step);
            assert!((bend[axis] - approximate_bend).abs() < 1e-5);
        }
    }
}
...
fn empty_trajectory_is_an_error() {
    let no_pieces: &[[Polynomial<4>; 2]] = &[];
    assert_eq!(
        PiecewisePolynomial::<2, 4, 2, f64>::try_from_pieces(no_pieces, &[]).err(),
        Some(PolynomialError::Empty)
    );
}

#[test]
fn rejects_a_non_positive_span() {
    let piece = [Polynomial::<4>::new([1.0, 0.0, 0.0, 0.0]); 2];
    for span in [0.0, -1.0, f64::NAN] {
        assert_eq!(
            PiecewisePolynomial::<2, 4, 2, f64>::try_from_pieces(&[piece], &[span]).err(),
            Some(PolynomialError::SpanNotPositive)
        );
    }
    // One span per piece, no more and no fewer.
    assert_eq!(
        PiecewisePolynomial::<2, 4, 2, f64>::try_from_pieces(&[piece], &[1.0, 1.0]).err(),
...
fn accessors_report_what_was_built() {
    ...
    assert!((curve.total_span() - 2.5).abs() < 1e-12);

    assert!((curve.span(0).unwrap() - 2.0).abs() < 1e-12);
    assert!((curve.span(1).unwrap() - 0.5).abs() < 1e-12);
    assert_eq!(curve.span(2), None);

    assert_eq!(
        curve.piece_polynomial(0, 1).unwrap().coefficients(),
        &[1.0, 0.0, 2.0, -1.0]
    );
    assert_eq!(curve.piece_polynomial(0, 2), None);
    assert_eq!(curve.piece_polynomial(2, 0), None);
}

#[test]
fn holds_cubic_pieces_too() {
    // Four coefficients per piece rather than eight, so the type is not pinned to one degree.
    let first_axis = Polynomial::<4>::from_endpoint_derivatives(0.0, 0.0, 1.0, 0.5, 2.0).unwrap();
    let second_axis = Polynomial::<4>::from_endpoint_derivatives(1.0, 0.0, 3.0, 0.0, 2.0).unwrap();
    let curve =
        PiecewisePolynomial::<3, 4, 2, f64>::try_from_pieces(&[[first_axis, second_axis]], &[2.0])
            .unwrap();

    let at_start = curve.evaluate(0.0).unwrap();
    assert!(at_start[0].abs() < 1e-12 && (at_start[1] - 1.0).abs() < 1e-12);

    let at_end = curve.evaluate(2.0).unwrap();
    assert!((at_end[0] - 1.0).abs() < 1e-12 && (at_end[1] - 3.0).abs() < 1e-12);
}
