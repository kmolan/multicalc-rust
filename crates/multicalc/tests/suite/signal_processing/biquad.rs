...
fn reported_magnitude_matches_a_long_run() {
    ...
    assert!((largest - coefficients.magnitude_at(30.0).unwrap()).abs() < 0.02);
}

#[test]
fn response_queries_reject_a_frequency_at_or_above_nyquist() {
    let coefficients = BiquadCoefficients::low_pass(50.0_f64, FLATTEST, 0.001).unwrap();
...
// Below Nyquist, and at the zero delay_at special-cases, the response queries still agree with
// what they reported before they could fail.
#[test]
fn response_queries_agree_with_today_below_nyquist() {
    let coefficients = BiquadCoefficients::low_pass(50.0_f64, FLATTEST, 0.001).unwrap();
    assert!((coefficients.magnitude_at(50.0).unwrap() - 1.0 / 2.0_f64.sqrt()).abs() < 0.02);
    assert!(coefficients.magnitude_at(400.0).unwrap() < 0.05);
    assert_eq!(coefficients.delay_at(0.0), Ok(0.0));
    assert!(coefficients.delay_at(50.0).unwrap() > 0.0);
}
...
fn non_finite_input_spoils_filter() {
    // A non-finite input spoils the Biquad filter till its reset
