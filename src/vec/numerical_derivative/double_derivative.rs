...
///
/// use multicalc::vec::numerical_derivative::double_derivative;
/// 
...
///``` 
/// 
pub fn get_partial<T: ComplexFloat>(func: &dyn Fn(&Vec<T>) -> T, idx_to_derivate: &[usize; 2], point: &Vec<T>) -> Result<T, ErrorCode>
{
...
/// NumberOfStepsCannotBeZero -> if the derivative step size is zero
/// IndexToDerivativeOutOfRange -> if the value of idx_to_derivate is greater than the number of variables
pub fn get_partial_custom<T: ComplexFloat>(func: &dyn Fn(&Vec<T>) -> T, idx_to_derivate: &[usize; 2], point: &Vec<T>, step: f64, mode: mode::DiffMode) -> Result<T, ErrorCode>
{
    match mode
    {
...
pub fn get_forward_difference<T: ComplexFloat>(func: &dyn Fn(&Vec<T>) -> T, idx_to_derivate: &[usize; 2], point: &Vec<T>, step: f64) -> Result<T, ErrorCode>
{
...
pub fn get_backward_difference<T: ComplexFloat>(func: &dyn Fn(&Vec<T>) -> T, idx_to_derivate: &[usize; 2], point: &Vec<T>, step: f64) -> Result<T, ErrorCode>
{
...
pub fn get_central_difference<T: ComplexFloat>(func: &dyn Fn(&Vec<T>) -> T, idx_to_derivate: &[usize; 2], point: &Vec<T>, step: f64) -> Result<T, ErrorCode>
{
    let mut f0_point = point.clone();
    f0_point[idx_to_derivate[0]] = f0_point[idx_to_derivate[0]] - T::from(step).unwrap();
