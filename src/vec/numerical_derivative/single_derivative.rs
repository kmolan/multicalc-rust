...
/// 
/// use multicalc::vec::numerical_derivative::single_derivative;
///
...
///```
/// 
pub fn get_partial<T: ComplexFloat>(func: &dyn Fn(&Vec<T>) -> T, idx_to_derivate: usize, point: &Vec<T>) -> Result<T, ErrorCode>
{
...
/// NumberOfStepsCannotBeZero -> if the derivative step size is zero
/// IndexToDerivativeOutOfRange -> if the value of idx_to_derivate is greater than the number of variables
pub fn get_partial_custom<T: ComplexFloat>(func: &dyn Fn(&Vec<T>) -> T, idx_to_derivate: usize, point: &Vec<T>, step: f64, mode: mode::DiffMode) -> Result<T, ErrorCode>
{
    if step == 0.0
    {
        return Err(ErrorCode::NumberOfStepsCannotBeZero);
