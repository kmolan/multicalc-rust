...
/// ```
pub fn get_3d<T: ComplexFloat>(vector_field: &[Box<dyn Fn(&Vec<T>) -> T>; 3], point: &Vec<T>) -> T
{
...
pub fn get_3d_custom<T: ComplexFloat>(vector_field: &[Box<dyn Fn(&Vec<T>) -> T>; 3], point: &Vec<T>, step_size: f64, mode: mode::DiffMode) -> T
{
