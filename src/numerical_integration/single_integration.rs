...
pub fn get_simple(integration_method: IntegrationMethod, func: &dyn Fn(&Vec<f64>) -> f64, integration_interval: &[f64; 2], n: u64) -> f64
{
    ...
    {
        IntegrationMethod::Booles        => return get_booles(func, 0, integration_interval, &point, n),
        IntegrationMethod::GaussLegendre => return get_gauss_legendre(func, 0, integration_interval, &point, n as usize),
        IntegrationMethod::Simpsons      => return get_simpsons(func, 0, integration_interval, &point, n),
        IntegrationMethod::Trapezoidal   => return get_trapezoidal(func, 0, integration_interval, &point, n)
    }
}
...
/// 
//// if we then want to integrate this function over y with 100 steps over the interval [0.0, 2.0], we would use:
/// 
/// use multicalc::numerical_integration::mode::IntegrationMethod;
/// use multicalc::numerical_integration::single_integration;
///
...
/// Note: The argument 'n' denotes the number of steps to be used. However, for [`IntegrationMethod::GaussLegendre`], it denotes the highest order of our equation
/// 
pub fn get_partial(integration_method: IntegrationMethod, func: &dyn Fn(&Vec<f64>) -> f64, idx_to_integrate: usize, integration_interval: &[f64; 2], point: &Vec<f64>, n: u64) -> f64
{
    match integration_method
    {
        IntegrationMethod::Booles        => return get_booles(func, idx_to_integrate, integration_interval, point, n),
        IntegrationMethod::GaussLegendre => return get_gauss_legendre(func, idx_to_integrate, integration_interval, point, n as usize),
        IntegrationMethod::Simpsons      => return get_simpsons(func, idx_to_integrate, integration_interval, point, n),
        IntegrationMethod::Trapezoidal   => return get_trapezoidal(func, idx_to_integrate, integration_interval, point, n)
    }
}
...
fn get_booles(func: &dyn Fn(&Vec<f64>) -> f64, idx_to_integrate: usize, integration_interval: &[f64; 2], point: &Vec<f64>, steps: u64) -> f64
{
    ...
    let delta = (integration_interval[1] - integration_interval[0])/(steps as f64);
    ...
    current_vec[idx_to_integrate] = integration_interval[1];
...
fn get_gauss_legendre(func: &dyn Fn(&Vec<f64>) -> f64, idx_to_integrate: usize, integration_interval: &[f64; 2], point: &Vec<f64>, order: usize) -> f64
{
    ...
    let intercept = (integration_interval[1] + integration_interval[0])/2.0;
