...
pub trait IntegratorSingleVariable: Default + Clone + Copy
{
    ...
    fn get_single<T:ComplexFloat>(&self, func: &dyn Fn(T) -> T, integration_limit: &[T; 2]) -> Result<T, &'static str>
    {
    ...
    fn get_double<T:ComplexFloat>(&self, func: &dyn Fn(T) -> T, integration_limit: &[[T; 2]; 2]) -> Result<T, &'static str>
    {
...
pub trait IntegratorMultiVariable : Default + Clone + Copy
{
    ...
    fn get_single_partial<T:ComplexFloat, const NUM_VARS: usize>(&self, func: &dyn Fn(&[T; NUM_VARS]) -> T, idx_to_integrate: usize, integration_limits: &[T; 2], point: &[T; NUM_VARS]) -> Result<T, &'static str>
    {
