...
- **Fast, and measured**: a derivative in **~1 ns**, a full Levenberg-Marquardt curve fit in
  **microseconds**, and solvers that land on the answer to the **last few bits** (objectives near
  `1e-30`, errors within ~1 ulp). Accuracy is enforced against proven external libraries like `numpy` and `scipy`. See the [benchmarks](tools/qa/benches).
...
- **[QA crate](tools/qa)**: `multicalc-qa` holds the CI-enforced accuracy fixtures [benchmarks](tools/qa/benches).
